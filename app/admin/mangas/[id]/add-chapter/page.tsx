...
import { 
  ArrowLeft, Loader2, UploadCloud, Trash2, Sparkles, FileText, CheckCircle2
} from "lucide-react";
...
export default function AddChapterPage(props: AddChapterPageProps) {
  ...
  // Manga detayını yükle ve yetkiyi kontrol et
  useEffect(() => {
    ...
    const fetchManga = async () => {
      try {
        ...
        console.error("Manga detayları çekilirken hata:", err);
      } finally {
  ...
  // Çoklu Dosya Yükleme & Sıralı Base64 Dönüşümü
  const handleFilesChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setMessage({ text: err.message || "Görseller yüklenirken bir hata oluştu.", isError: true });
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (pages.length === 0) {
      setMessage({ text: "Lütfen en az bir adet sayfa resmi yükleyin.", isError: true });
      setLoading(false);
      return;
    }

    try {
      const res = await fetch(`/api/mangas/${id}/chapters`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Bölüm yüklenirken bir hata oluştu.");
      }

      setMessage({ text: "Bölüm başarıyla yüklendi ve Discord duyurusu yapıldı!", isError: false });
      
      // Manga detay sayfasına yönlendir
      setTimeout(() => {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-400 font-light mt-1.5">
                  {manga ? `"${manga.title}"` : "Manga"} serisi için yeni bir bölüm ve okuma sayfaları oluşturun.
                </p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="block text-[10px] font-mono uppercase tracking-wider text-zinc-400">
                Bölüm Sayfaları (Sıralı Görseller) <span className="text-red-500">*</span>
              </label>
            ...
            {message && (
              ...
              >
                {!message.isError && <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
                {message.text}
              </div>
            )}
            ...
            >
              {loading ? (
                <>
                  ...
                  Bölüm Yayınlanıyor ve Bildirim Gönderiliyor...
                </>
              ) : (
                <>
                  <Sparkles className="w-4 h-4 text-black animate-pulse" />
                  Bölümü Yayınla & Discord Bildirimi Gönder
                </>
              )}
            </button>
          </form>
